...
public class BinaryReadDemo {
	...
	public static void main(String[] args) {
		...
		try {
			...
			/*
			 * int ch = 0; while(true) { ch = istream.read(); //reading a byte from stream
			 * if(ch==1) //when end of file reached break; //exit loop
			 * System.out.println((char)ch); }
			 */
			byte[] content = new byte[istream.available()]; // returns count of available bytes in stream
			istream.read(content); // reading bytes from stream to array
